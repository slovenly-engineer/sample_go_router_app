...
**推奨: iPhone 15 Pro (393 × 852 pt)**

このプロジェクトはiOSユーザーが多いため、iPhone 15 Pro を基準サイズとして採用します。

| 項目 | 値 |
|------|-----|
| **Width** | 393 pt |
| **Height** | 852 pt |
| **Aspect Ratio** | 19.5:9 (縦長) |
| **ブレークポイント** | Compact (〜600pt) |
| **デバイス** | iPhone 15 Pro |

#### なぜこのサイズなのか

1. **最新のiOS標準**: 2024-2026年の主流デバイスサイズ
2. **縦長画面**: 最近のUIトレンド（ノッチ、ボトムナビ）に最適
3. **Material Design 3対応**: Compact ブレークポイントに該当
4. **iOS + Android 互換**: Flutterのレスポンシブ機能で両プラットフォームに対応可能

### 15.2 テスト確認用サイズ

設計完了後、以下のサイズでプレビュー確認を推奨します：

| デバイス | サイズ (pt) | 用途 |
|---------|------------|------|
| **小型Android** | 360 × 800 | 最小サイズでのレイアウト崩れ確認 |
| **iPhone 15 Pro Max** | 430 × 932 | 大型iOSでの表示確認、余白・スペーシング調整 |
| **Android Pixel** | 412 × 915 | Android標準での確認、Material Design動作検証 |

#### レスポンシブ対応の確認ポイント

- [ ] 小型Android (360×800) でスクロールが正常に動作
- [ ] テキストが途切れず、オーバーフローが発生しない
- [ ] タップ領域が最小44×44ptを維持
- [ ] 大型デバイスで余白が適切に配置されている

### 15.3 Pencilでの設定

#### 新規ドキュメント作成時

Pencilで新規ドキュメントを作成する際は、以下の設定を使用してください：

```
Width:  393 pt
Height: 852 pt
Device Frame: iPhone 15 Pro (オプション)
```
...
2. プロパティパネルで以下を設定：
   - Width: `393`
   - Height: `852`
3. Constraints（制約）を確認し、レスポンシブ対応を設定

#### レスポンシブ設計のヒント

Pencilでデザインする際は、以下を意識してください：

- **固定要素**: AppBar、BottomNavigationBarは高さ固定
- **可変要素**: メインコンテンツはスクロール可能に設計
- **制約**: 要素の最大幅・最小幅を考慮（`ConstrainedBox`に対応）
- **スペーシング**: 8dpの倍数を使用（セクション4参照）

#### Pencil → Flutter実装時の注意

Pencilで固定サイズ (393×852) でデザインしても、Flutter実装時は以下のようにレスポンシブ対応します：

```dart
// ❌ Pencilデザインをそのまま固定サイズで実装しない
Container(
  width: 393,
  height: 852,
  child: ...,
...
Container(
  ...
  constraints: BoxConstraints(maxWidth: 600), // 大画面対応
  child: SingleChildScrollView(  // 小画面でスクロール
    child: ...,
  ),
)
```
